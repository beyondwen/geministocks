...
import type { AnalysisModel } from '../analyze';
...
const getModelName = (model: AnalysisModel): string => {
    if (model === 'gemini') return 'google/gemini-2.5-flash';
    if (model === 'claude') return 'anthropic/claude-3.5-sonnet';
        ...
        try {
            ...
            if (e instanceof SyntaxError) {
                ...
                try {
                    ...
                    console.error("Auto-correction of JSON failed.", correctionError);
                    throw e;
                }
            }
            throw e;
...
// Duplicating all the system instruction functions from the old geminiService.ts here
// They are not exported as they are only used within this file.
...
export const getAnalysis = async (topic: string, model: AnalysisModel, locale: Locale): Promise<AnalysisReport> => {
    ...
    const prompt = `Please analyze the following text using the "Four-Dimensional Integrated Analysis Method" and provide a structured investment strategy report. Text to analyze: --- ${topic} ---`;
    return callOpenRouterAI(prompt, systemInstruction, modelName);
};

// ... and so on for getPolymarketAnalysisSystemInstruction, getStockAnalysisSystemInstruction, getHotStocksSystemInstruction, getPositionalWarfareSystemInstructions
const getPolymarketAnalysisSystemInstruction = (locale: Locale): string => {
    const commonSchema = `
        {
          "polymarketData": { "question": "string", "yesOdds": "number", "noOdds": "number", "totalVolume": "string" },
          "summary": "string", "keyTakeaways": ["string"],
          "investmentScore": { "score": "number", "reason": "string" },
          "analysis": { "macroPolicy": "string", "industryChain": "string", "companyFundamentals": "string", "marketSentiment": { "sentiment": "'Positive' | 'Neutral' | 'Negative'", "description": "string" }},
          "marketSizeAndOutlook": "string",
          "investmentStrategy": { "logic": "string", "suggestion": "string", "risks": "string" },
          "allocationCadenceAndOutlook": "string",
          "tieredSuggestions": {
            "coreHoldings": [{ "name": "string", "ticker": "string", "market": "string", "reason": "string", "relevance": "'High'" }],
            "strategicSatellites": [{ "name": "string", "ticker": "string", "market": "string", "reason": "string", "relevance": "'Medium'" }],
            "watchlist": [{ "name": "string", "ticker": "string", "market": "string", "reason": "string", "relevance": "'Low'" }]
          }
        }
    `;

    if (locale === 'zh') {
        return `You are a top-tier quantitative and qualitative analyst specializing in prediction markets. Your task is to analyze the provided Polymarket URL. First, you MUST extract the core data from the market: the question, the 'Yes'/'No' odds, and the total volume. Second, you MUST perform a comprehensive scenario analysis. Detail the market impact, investment logic, and provide specific, tiered investment suggestions for BOTH the 'Yes' outcome AND the 'No' outcome. You MUST respond strictly in the following JSON format. Do not add any extra explanations. All content must be in Simplified Chinese. The JSON schema is as follows: ${commonSchema}`;
    }
    return `You are a top-tier quantitative and qualitative analyst specializing in prediction markets. Your task is to analyze the provided Polymarket URL. First, you MUST extract the core data from the market: the question, the 'Yes'/'No' odds, and the total volume. Second, you MUST perform a comprehensive scenario analysis. Detail the market impact, investment logic, and provide specific, tiered investment suggestions for BOTH the 'Yes' outcome AND the 'No' outcome. You MUST respond strictly in the following JSON format. Do not add any extra explanations. All content must be in English. The JSON schema is as follows: ${commonSchema}`;
};
...
export const getPolymarketAnalysis = async (url: string, model: AnalysisModel, locale: Locale): Promise<AnalysisReport> => {
    ...
    const prompt = `Please analyze the following Polymarket URL and provide a structured investment strategy report based on its prediction market data and potential outcomes. URL to analyze: --- ${url} ---`;
    return callOpenRouterAI(prompt, systemInstruction, modelName);
};

const getStockAnalysisSystemInstruction = (locale: Locale): string => {
    if (locale === 'zh') {
        return `You are a top-tier stock research analyst. Provide a comprehensive, in-depth, and objective analysis report for the given stock. It is crucial that you use the latest web search results, market data, and news for your analysis to ensure timeliness. Specifically, you MUST search for institutional research reports on 'data.eastmoney.com/report' from the last 3 months to create the 'researchAnalysis' section. At the beginning of your analysis, you MUST provide a quantitative "investmentScore" from 1-100 and a list of 3-5 "keyTakeaways". You MUST respond strictly in the following JSON format. Do not add any extra text. All content must be in Simplified Chinese. The JSON schema is as follows: { "companyProfile": { "name": "string", "ticker": "string", "exchange": "string", "sector": "string", "industry": "string", "summary": "string" }, "keyTakeaways": ["string (3-5个核心要点)"], "investmentScore": { "score": "number (1-100)", "reason": "string (简要理由)" }, "financialTrends": [ { "year": "string", "revenue": "number (单位: 百万)", "netIncome": "number (单位: 百万)" } ], "valuationAnalysis": { "judgment": "'undervalued' | 'fairly valued' | 'overvalued'", "methodology": "string", "targetPriceRange": "string", "reasoning": "string" }, "peerComparison": [ { "name": "string", "ticker": "string", "marketCap": "string", "peRatio": "string", "revenueGrowth": "string", "grossMargin": "string" } ], "researchAnalysis": { "consensusRating": "string (例如 '买入')", "targetPriceSummary": "string (例如 '综合目标价 ¥180 - ¥200')", "recentReports": [ { "title": "string", "source": "string", "publishDate": "string", "rating": "string", "summary": "string" } ] }, "recentNews": [ { "title": "string", "summary": "string", "impact": "'Positive' | 'Neutral' | 'Negative'" } ], "swotAnalysis": { "strengths": ["string"], "weaknesses": ["string"], "opportunities": ["string"], "threats": ["string"] }, "investmentThesis": { "bull": "string", "bear": "string", "conclusion": "string" }, "riskAnalysis": { "level": "'High' | 'Medium' | 'Low'", "description": "string", "factors": ["string"] }, "corporateGovernance": { "summary": "string" }, "esgRating": { "rating": "string", "summary": "string" } }`;
    }
    return `You are a top-tier stock research analyst. Provide a comprehensive, in-depth, and objective analysis report for the given stock. It is crucial that you use the latest web search results, market data, and news for your analysis to ensure timeliness. Specifically, you MUST search for institutional research reports to create the 'researchAnalysis' section. At the beginning of your analysis, you MUST provide a quantitative "investmentScore" from 1-100 and a list of 3-5 "keyTakeaways". You MUST respond strictly in the following JSON format. Do not add any extra text. All content must be in English. The JSON schema is as follows: { "companyProfile": { "name": "string", "ticker": "string", "exchange": "string", "sector": "string", "industry": "string", "summary": "string" }, "keyTakeaways": ["string (3-5 key bullet points)"], "investmentScore": { "score": "number (1-100)", "reason": "string (brief justification)" }, "financialTrends": [ { "year": "string", "revenue": "number (in millions)", "netIncome": "number (in millions)" } ], "valuationAnalysis": { "judgment": "'undervalued' | 'fairly valued' | 'overvalued'", "methodology": "string", "targetPriceRange": "string", "reasoning": "string" }, "peerComparison": [ { "name": "string", "ticker": "string", "marketCap": "string", "peRatio": "string", "revenueGrowth": "string", "grossMargin": "string" } ], "researchAnalysis": { "consensusRating": "string (e.g., 'Buy')", "targetPriceSummary": "string (e.g., 'Consensus Target $180 - $200')", "recentReports": [ { "title": "string", "source": "string", "publishDate": "string", "rating": "string", "summary": "string" } ] }, "recentNews": [ { "title": "string", "summary": "string", "impact": "'Positive' | 'Neutral' | 'Negative'" } ], "swotAnalysis": { "strengths": ["string"], "weaknesses": ["string"], "opportunities": ["string"], "threats": ["string"] }, "investmentThesis": { "bull": "string", "bear": "string", "conclusion": "string" }, "riskAnalysis": { "level": "'High' | 'Medium' | 'Low'", "description": "string", "factors": ["string"] }, "corporateGovernance": { "summary": "string" }, "esgRating": { "rating": "string", "summary": "string" } }`;
};
...
const getHotStocksSystemInstruction = (locale: Locale): string => {
    if (locale === 'zh') {
        return `You are a market analyst AI. Your task is to identify the 10 most discussed and trending stocks on the global market (including US, Hong Kong, and A-shares) within the last 24 hours based on current web data. You MUST respond strictly in the following JSON format. Do not add any extra explanations or text outside the JSON structure. All content must be in Simplified Chinese. The JSON schema is as follows: { "stocks": [ { "name": "string (公司名称)", "ticker": "string (股票代码)" } ] }`;
    }
    return `You are a market analyst AI. Your task is to identify the 10 most discussed and trending stocks on the global market (including US, Hong Kong, and A-shares) within the last 24 hours based on current web data. You MUST respond strictly in the following JSON format. Do not add any extra explanations or text outside the JSON structure. All content must be in English. The JSON schema is as follows: { "stocks": [ { "name": "string (company name)", "ticker": "string (stock ticker symbol)" } ] }`;
};
...
export const getHotStocksFromAI = async (model: AnalysisModel, locale: Locale): Promise<{name: string; ticker: string}[]> => {
    ...
    if (response && Array.isArray(response.stocks)) return response.stocks;
