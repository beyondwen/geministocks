...
export const getAnalysis = async (topic: string): Promise<AnalysisReport> => {
  // FIX: API key is retrieved from environment variables as per guidelines.
  const apiKey = process.env.API_KEY;
  if (!apiKey) {
    throw new Error("Gemini API 密钥未在环境变量中配置。🔑");
  }
  ...
  try {
    ...
    // It's good practice to handle potential markdown code fences
    const cleanedJsonText = jsonText.replace(/^```json\s*|```$/g, '');
    const result = JSON.parse(cleanedJsonText);
    return result as AnalysisReport;
  } catch (e) {
    console.error("Failed to parse JSON response:", response.text);
    throw new Error("无法从 AI 响应中解析分析报告。😵");
  }
};